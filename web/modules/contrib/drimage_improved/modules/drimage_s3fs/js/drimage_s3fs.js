(function (document, Drupal, drupalSettings) {
  ...
  Drupal.drimage_improved.init = function (context) {
    ...
    var el = context.querySelectorAll('.s3_drimage:not(.is-loading)');
    if (el.length > 0) {
  ...
  function handleResize() {
    const images = document.querySelectorAll('.s3_drimage:not(.is-loading)');
    images.forEach(img => {
  ...
  Drupal.behaviors.drimage_improved = {
    ...
    attach: function (context) {
      ...
      const images = context.querySelectorAll('.s3_drimage:not(.is-loading)');
      images.forEach(img => {
